...
t.test(britishSeatBeltStudy$FractionOfDriversKilled [which(britishSeatBeltStudy$law %in% c(1))], britishSeatBeltStudy$FractionOfDriversKilled [which(britishSeatBeltStudy$law %in% c(9))], var.equal = F, paired = F)
...
#Linear Analysis: Linear Regression to predict the relationship between drivers killed and the speed, petrol price, and seatbelt law
